...
/**
 * Shared nominations list for Admin and Agent. Agent ẩn cột Admin phụ trách và nút Memo.
 * @param {'admin'|'agent'} variant
 */
const NominationsPageContent = ({ variant = 'admin' }) => {
  ...
  useEffect(() => {
    loadNominations();
  }, [currentPage, itemsPerPage, statusFilter, dateFrom, dateTo, sortBy, sortOrder, viewMode, adminProfile?.id, searchQuery]);
  ...
  const mapAgentStatus = (status) => {
    ...
    let key = 'pending';
    if (statusInfo.category === 'success' || status === 8 || status === 11) key = 'accepted';
  ...
  const agentMappedNominations = useMemo(() => {
    ...
    const mapped = rawJobApplications.map((app) => {
      ...
      const job = app.job;
      const jobTitle = pickByLanguage(job, 'title') || pickByLanguageFields(job?.title, job?.titleEn || job?.title_en, job?.titleJp || job?.title_jp, language) || '—';
  ...
  const SortableTh = ({ label, sortKey }) => (
    <th
      className="px-3 py-2 text-left text-[10px] font-semibold border-b cursor-pointer select-none hover:bg-gray-100 transition-colors"
    ...
    <div className="h-full flex flex-col overflow-hidden">
      ...
      <div className="w-full py-1.5 mb-1.5 flex-shrink-0">
        <div className="flex items-center gap-2.5 flex-wrap justify-between">
          {/* Search box (rounded-full, không border) */}
          ...
          <div className="flex items-center gap-2.5 flex-wrap justify-end">
            ...
            <div className="relative">
              ...
              {isStatusFilterOpen && (
                <div className="absolute right-0 mt-2 w-52 rounded-xl border bg-white p-3 z-20 text-[11px] sm:text-xs max-h-60 overflow-y-auto" style={{ borderColor: '#e5e7eb' }}>
                  <div className="flex flex-col gap-2">
            ...
            <button
              ...
              className="px-3 py-1.5 rounded-full font-semibold text-[11px] sm:text-xs transition-colors bg-white"
              style={{
            ...
            {isAdmin && (
              <>
                <button
                  ...
                  className="px-3 py-1.5 rounded-full font-semibold text-[11px] sm:text-xs transition-colors border"
                  style={{
                ...
                >
                  {viewMode === 'myAssigned' ? t.viewAllNominations : t.viewMyAssignedOnly}
                </button>
                <button
                  onClick={() => navigate(`${basePath}/nominations/create`)}
                  onMouseEnter={() => setHoveredAddNominationButton(true)}
                  onMouseLeave={() => setHoveredAddNominationButton(false)}
                  className="px-3 sm:px-4 py-1.5 rounded-full text-[11px] sm:text-xs font-semibold"
                  style={{
        ...
        {isAdmin && adminProfile && (
          <p className="text-[11px] mt-1 px-1" style={{ color: '#6b7280' }}>
            {viewMode === 'myAssigned' ? t.viewingMyAssigned : t.viewingAll}
          </p>
        )}
        {!isAdmin && agentLoadError && (
          <p className="text-[11px] mt-1 px-1" style={{ color: '#dc2626' }}>{agentLoadError}</p>
        )}
      </div>

      {/* Pagination Controls */}
      <div className="flex items-center justify-between mb-3 flex-shrink-0 text-[10px]">
        <div className="flex items-center gap-1.5">
          <button
            ...
            className="px-1.5 py-1 border rounded text-[10px] font-semibold transition-colors"
            style={{
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-[10px] font-semibold" style={{ color: '#374151' }}>{totalItems} items</span>
        </div>
      </div>

      {/* Table – không nền chung, mỗi dòng có nền riêng */}
          ...
          <table className="w-full" style={{ borderCollapse: 'separate', borderSpacing: '0 4px' }}>
            <thead className="sticky top-0 z-10" style={{ backgroundColor: '#f9fafb' }}>
              <tr>
                ...
                <th className="px-3 py-2 text-left text-[10px] font-semibold border-b" style={{ color: '#111827', borderColor: '#e5e7eb' }}>{t.colId}</th>
                <SortableTh label={t.colCandidate} sortKey="candidate_name" />
                <th className="px-3 py-2 text-left text-[10px] font-semibold border-b" style={{ color: '#111827', borderColor: '#e5e7eb' }}>{t.colJob}</th>
                {isAdmin && (
                  <th className="px-3 py-2 text-left text-[10px] font-semibold border-b" style={{ color: '#111827', borderColor: '#e5e7eb' }}>{t.colAdminResponsible}</th>
                )}
                <th className="px-3 py-2 text-left text-[10px] font-semibold border-b" style={{ color: '#111827', borderColor: '#e5e7eb' }}>{t.colStatus}</th>
            ...
            <tbody>
              {loading ? (
                <tr style={{ backgroundColor: '#ffffff', boxShadow: '0 1px 3px rgba(0,0,0,0.06)' }}>
                  <td colSpan={isAdmin ? 8 : 7} className="px-3 py-8 text-center text-[10px] rounded-lg" style={{ color: '#6b7280' }}>
                    {t.loadingData}
                  </td>
                </tr>
                ...
                displayList.map((nomination, index) => {
                  ...
                  const jobTitle = isAdminRow ? (pickByLanguage(nomination.job, 'title') || nomination.job?.jobCode || '-') : (nomination.jobTitle || '—');
                  const companyName = isAdminRow ? (nomination.job?.recruitingCompany?.companyName || '—') : (nomination.companyName || '—');
                  const cvId = isAdminRow ? nomination.cvId : nomination.cvId;
                  ...
                  return (
                    <tr
                      ...
                      style={{
                        backgroundColor: hoveredRowIndex === index ? '#f9fafb' : '#ffffff',
                        boxShadow: '0 1px 3px rgba(0,0,0,0.06)'
                      }}
                      onMouseEnter={() => setHoveredRowIndex(index)}
                      onMouseLeave={() => setHoveredRowIndex(null)}
                    >
                      <td className="px-3 py-2 text-center" style={{ borderTopLeftRadius: 8, borderBottomLeftRadius: 8 }}>
                        <input
                          type="checkbox"
                      ...
                      <td className="px-3 py-2">
                        <div className="flex items-center gap-1.5">
                          <button
                            onClick={() => navigate(`${basePath}/nominations/${nomination.id}`)}
                      ...
                      <td className="px-3 py-2">
                        <div className="flex items-center gap-1.5">
                          <div className="w-6 h-6 rounded-full flex items-center justify-center text-white font-semibold text-[9px]" style={{ backgroundColor: '#a855f7' }}>
                            {candidateName.charAt(0).toUpperCase()}
                          </div>
                          <div>
                            {cvId ? (
                              <button
                                onClick={() => navigate(`${basePath}/candidates/${cvId}`)}
                            ...
                            {isAdminRow && (nomination.collaborator?.code || nomination.collaborator?.name) && (
                            ...
                            {(unreadByJobApplication[nomination.id] || 0) > 0 && (
                              <span
                                className="inline-flex items-center mt-1 px-1.5 py-0.5 rounded text-[9px] font-semibold"
                      ...
                      <td className="px-3 py-2 max-w-[200px]">
                        {nomination.jobId ? (
                          <button
                            onClick={() => navigate(isAdminRow ? `${basePath}/jobs/${nomination.jobId}` : `${basePath}/jobs/${nomination.jobId}${nomination.jobSlug ? `?slug=${nomination.jobSlug}` : ''}`)}
                          ...
                          >
                            <span className="text-[10px] font-medium line-clamp-2">
                              {truncate(jobTitle, JOB_TITLE_MAX)}
                            </span>
                            <span className="text-[9px] block mt-0.5" style={{ color: '#6b7280' }}>
                              {companyName}
                            </span>
                          </button>
                        ) : (
                          ...
                          <div className="flex flex-col gap-1">
                            {nomination.adminResponsibleId ? (
                              <div className="flex items-center gap-1.5">
                                ...
                                <span className="text-[10px] font-medium" style={{ color: '#1e3a8a' }}>
                                  {nomination.adminResponsible?.name || nomination.adminResponsible?.email || '-'}
                                </span>
                              </div>
                            ) : (
                              <span className="text-[10px] font-medium" style={{ color: '#dc2626' }}>{t.notAssignedAdmin}</span>
                            )}
                            {isSuperAdmin && (
                              <div className="flex items-center gap-1 mt-0.5">
                                <select
                                  ...
                                  style={{ borderColor: '#d1d5db', minWidth: '120px' }}
                                >
                        ...
                        <div>
                          {isAdminRow ? (
                            <select
                              ...
                              className="w-full max-w-[200px] px-2 py-1 rounded text-[9px] font-semibold border cursor-pointer"
                              style={{
                          ...
                          {showInterviewDate && (
                            <p className="text-[9px] mt-1" style={{ color: '#6b7280' }}>PV: {interviewDateStr}</p>
                          )}
                        </div>
                      </td>
                      <td className="px-3 py-2 text-[10px]" style={{ color: '#374151' }}>
                        <div className="flex items-center gap-1">
                          <Calendar className="w-2.5 h-2.5 flex-shrink-0" style={{ color: '#9ca3af' }} />
                          {appliedDate}
                        </div>
                      </td>
                      <td className="px-3 py-2" style={{ borderTopRightRadius: 8, borderBottomRightRadius: 8 }}>
                        <div className="flex items-center justify-center gap-1.5">
                          {isAdmin && (isSuperAdmin || isOwner) && (
                            <button
                              ...
                              className="relative px-2 py-0.5 rounded text-[10px] font-semibold border transition-colors"
                              style={{
                                borderColor: '#fecaca',
                          ...
                          {isAdmin && (isSuperAdmin || isOwner) && (
                            <>
                              <button
                                onClick={() => navigate(`${basePath}/nominations/${nomination.id}/edit`)}
